...
function truncateToFixedString(value, decimals) {
    ...
    var truncatedInteger = Math.trunc(absValue * factor); // toward zero
    var integerPart = Math.trunc(truncatedInteger / factor).toString();
    var fractionalPart = (truncatedInteger % factor).toString().padStart(decimals, '0');

    return sign + integerPart + '.' + fractionalPart;
}

// 📌 3. SUPER_BIT 글로벌 변수 및 업데이트 함수
let SUPER_BIT = truncateToFixedString(0, 10);
...
function BIT_MAX_NB(nb, bit = 5.5) {
    ...
    if (!isFinite(result) || isNaN(result) || result > 100 || result < -100) {
        ...
        const formatted = truncateToFixedString(result, 10);
        updateSuperBit(formatted);
        return formatted;
    }
}
...
function calculateInclusionFromBase(sentence1, sentence2) {
    ...
    const clean = str => str.replace(/[^\wㄱ-ㅎ가-힣\s]/g, '').replace(/\s+/g, ' ').trim();
...
function wordNbUnicodeFormat(domain) {
  // 기본 문자열 설정
  const defaultPrefix = '한 국 인 터 넷 . 한 국';
  ...
  // 문자열을 문자 배열로 변환
  const chars = Array.from(domain); // 또는 domain.split('')
