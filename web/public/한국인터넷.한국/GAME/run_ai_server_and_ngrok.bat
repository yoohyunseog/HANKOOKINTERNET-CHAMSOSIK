...
start "Ollama AI Proxy Server" cmd /k "cd /d "%AI_SERVER_DIR%" && set "PORT=%PORT%" && set "HOST=%HOST%" && set "OLLAMA_URL=%OLLAMA_URL%" && set "OLLAMA_MODEL=%OLLAMA_MODEL%" && node server.js"

echo Waiting for server...
timeout /t 2 /nobreak >nul

echo Starting ngrok tunnel...
start "Ngrok AI Domain" cmd /k ""%NGROK_EXE%" http --url=%NGROK_DOMAIN% %PORT%"
